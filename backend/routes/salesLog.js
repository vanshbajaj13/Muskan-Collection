...
router.get("/search/:option/:query", protect, async (req, res) => {
  ...
  try {
    ...
    switch (searchField) {
      case "code":
        // Search by code
        items = await SaleLog.find({ code: { $regex: query, $options: "i" } });
        break;
      case "brand":
        // Search by brand
        items = await SaleLog.find({ brand: { $regex: query, $options: "i" } });
        break;
      case "product":
        // Search by product
        items = await SaleLog.find({ product: { $regex: query, $options: "i" } });
        break;
      case "category":
        // Search by category
        items = await SaleLog.find({ category: { $regex: query, $options: "i" } });
        break;
      case "size":
        // Search by size
        items = await SaleLog.find({ size: { $regex: query, $options: "i" } });
        break;
      default:
  ...
  try {
    ...
    switch (searchField) {
      case "code":
        ...
        items = await SaleLog.find({ code: { $regex: new RegExp(`^${query}$`, 'i') } });
        break;
      case "brand":
        // Search by brand (case-insensitive)
        items = await SaleLog.find({ brand: { $regex: new RegExp(`^${query}$`, 'i') } });
        break;
      case "product":
        // Search by product (case-insensitive)
        items = await SaleLog.find({ product: { $regex: new RegExp(`^${query}$`, 'i') } });
        break;
      case "category":
        // Search by category (case-insensitive)
        items = await SaleLog.find({ category: { $regex: new RegExp(`^${query}$`, 'i') } });
        break;
      case "size":
        // Search by size (case-insensitive)
        items = await SaleLog.find({ size: { $regex: new RegExp(`^${query}$`, 'i') } });
        break;
      default:
        return res.status(400).json({ message: "Invalid search option" });
    }
