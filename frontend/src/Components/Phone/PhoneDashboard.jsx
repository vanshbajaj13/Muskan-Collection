...
const PhoneDashboard = () => {
  ...
  const buildParams = useCallback(() => {
    ...
    if (filters.dateTo)   p.to   = new Date(filters.dateTo).setHours(23, 59, 59, 999);
    return p;
  ...
  useEffect(() => {
    ...
    const totalCommission = filtered.reduce((s, d) => s + (d.commissionAmount || 0), 0);
    ...
    const totalPending = filtered.reduce((s, d) => s + (d.paymentPending || 0), 0);
    ...
    const statusCounts = {
      ...
      pending_payment: filtered.filter((d) => d.dealStatus === "pending_payment").length,
        ...
        <div className="space-y-5">
          ...
          {!hasChartData && (
            <div className="text-center py-16 text-slate-400 bg-white rounded-xl border border-slate-200">
              ...
              <p className="font-medium text-slate-500">No completed deals for this filter</p>
              <p className="text-sm mt-1">Charts will appear once you have sold deals matching the current filters</p>
            </div>
          )}
        </div>
      )}
