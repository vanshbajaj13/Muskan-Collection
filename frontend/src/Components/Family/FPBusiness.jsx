...
export default function FPBusiness() {
  ...
  const openCreate = (year, month) => {
    const existing = businessEntries.find(
  ...
  const formStockTotal = form.stockPurchases.reduce(
    (s, p) => s + (Number(p.amount) || 0),
    0,
  );
  const formExpTotal = form.businessExpenses.reduce(
    (s, e) => s + (Number(e.amount) || 0),
    0,
  );
  const formProfit =
    (Number(form.salesRevenue) || 0) - formStockTotal - formExpTotal;
  ...
  const yearEntries = useMemo(
    () =>
      Array.from({ length: 12 }, (_, m) => {
        const entry = businessEntries.find(
          (b) => b.year === viewYear && b.month === m,
        );
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          {
            label: "Total Revenue",
            val: INR(yearTotals.revenue),
            color: "bg-emerald-50 text-emerald-700",
          },
          {
            label: "Stock Purchases",
            val: INR(yearTotals.stockExp),
            color: "bg-amber-50 text-amber-700",
          },
          {
            label: "Other Expenses",
            val: INR(yearTotals.otherExp),
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {row.entry?.notes && (
                          <span
                            className="text-xs text-slate-400 italic truncate max-w-[100px]"
                    ...
                    <td className="px-4 py-3 text-right">
                      {row.entry ? (
                        <span
                          className={`font-bold ${row.profit >= 0 ? "text-indigo-700" : "text-red-600"}`}
                        >
                          {row.profit >= 0 ? "+" : ""}
                          {INR(row.profit)}
                        </span>
            ...
            <tfoot>
              <tr className="bg-slate-50 font-bold text-sm">
                ...
                <td className="px-4 py-3 text-right text-emerald-700">
                  {INR(yearTotals.revenue)}
                </td>
                <td className="px-4 py-3 text-right text-amber-600">
                  {INR(yearTotals.stockExp)}
                </td>
                <td className="px-4 py-3 text-right text-rose-600">
                  {INR(yearTotals.otherExp)}
                </td>
                <td
                  className={`px-4 py-3 text-right ${yearTotals.profit >= 0 ? "text-indigo-700" : "text-red-600"}`}
                >
                  {yearTotals.profit >= 0 ? "+" : ""}
                  {INR(yearTotals.profit)}
                </td>
                <td />
              </tr>
            </tfoot>
        ...
        >
          <div className="space-y-5">
            <div className="bg-amber-50 rounded-lg px-4 py-3 text-sm text-amber-700 border border-amber-200">
              📅{" "}
              <strong>
                {MONTHS[Number(form.month)]} {form.year}
              </strong>
            </div>
            ...
            <Field
              label="Sales Revenue (₹)"
              hint="Total revenue from shop / business sales this month"
            >
              <FPInput
                type="number"
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Btn
                  variant="ghost"
                  className="text-xs px-2 py-1 text-indigo-600 hover:bg-indigo-50"
                ...
                {form.stockPurchases.map((item, idx) => (
                  <LineItemRow
                    ...
                    onChange={(i, field, val) =>
                      changeLineItem("stockPurchases", i, field, val)
                    }
              ...
              <div className="space-y-2">
                {form.businessExpenses.map((item, idx) => (
                  <LineItemRow
                    ...
                    index={idx}
                    onChange={(i, field, val) =>
                      changeLineItem("businessExpenses", i, field, val)
                    }
            ...
            {(form.salesRevenue || formStockTotal > 0 || formExpTotal > 0) && (
              ...
              >
                Net Profit Preview: {formProfit >= 0 ? "+" : ""}
                {INR(formProfit)}
                <span className="text-xs font-normal ml-2 opacity-70">
                  (Revenue {INR(Number(form.salesRevenue) || 0)} − Stock{" "}
                  {INR(formStockTotal)} − Other {INR(formExpTotal)})
                </span>
              </div>
            )}
      ...
      {saveConfirmOpen && editing && (
        <ConfirmModal
          title="Save Changes?"
          body={`Confirm changes to ${editing ? `${["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][editing.month]} ${editing.year}` : "this entry"}. Type CONFIRM to proceed.`}
